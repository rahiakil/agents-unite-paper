...
The economic direction is not ``everyone regenerates the market,'' but a three-path choice:

\begin{enumerate}[label=\arabic*.]
  \item \textbf{DIY (solo).} One operator pays full inference to cover $|T|$ tickers every day, plus any commercial research or alt-data subscriptions used as inputs. At \$0.05--\$0.50 per ticker (Table~\ref{tab:api-rates} plus harness), full-universe agent research alone is $\sim$\$200--\$2{,}000/day ($\sim$\$73k--\$730k/year at 365 days), before terminals or vendor feeds~\cite{openai-pricing,architecture}.
...
Table~\ref{tab:annual-stack} places these paths against published 2025--2026 list prices for common research and backtesting stacks. Agents Unite does \emph{not} replace a Bloomberg seat's full terminal workflow or SIP-grade price history; it targets the overlapping spend on \textbf{regenerating agent research} and \textbf{buying narrative/sentiment-like alt-data} that funds otherwise repurchase annually.
...
Quant and retail researchers typically pay twice for history: once for \emph{prices} (Table~\ref{tab:market-data}) and again, often invisibly, for \emph{features} built from news, social, and LLM synthesis. Without a shared ledger, each backtest window re-runs agents over past dates or re-licenses sentiment/news products. Agents Unite collapses the second bill: tagged dataset releases and \texttt{load\_reports.py} time series are free to fork, so a one-year sentiment factor study costs market-data API fees only, not $365 \times |T| \times c$ in regenerated tokens. Relative to institutional alt-data norms ($\sim$\$80k per dataset; $\sim$\$1.6M average annual buyer spend across $\sim$20 vendors~\cite{neudata2024}), an open Raft census is a lower-fidelity but zero-license complement for narrative features. Relative to a $\sim$\$32k/year terminal seat~\cite{bloomberg-cost}, the archive is not a drop-in replacement, but it removes the need to burn terminal-assisted research cycles regenerating the same ticker-day memos. Recent days in the ledger are especially valuable: they are immediately searchable for RAG and can be quality-matched or re-verified under PolyLink/VeriLLM-style inference attestations~\cite{polylink,verillm} without re-crawling the open web. The compounding effect is temporal: every contributed day permanently lowers the future cost of backtesting that date.
...
Researchers can correlate \texttt{sentiment\_score} with forward returns, train custom models on labeled sentiment, or embed reports for LLM terminals. The Hugging Face \texttt{DLT-Sentiment-News} dataset~\cite{dlt-sentiment} demonstrates demand for crowdsourced financial sentiment; Agents Unite provides a \emph{live, versioned} alternative. As Section~\ref{sec:monetary} notes, the monetary win for backtesting is avoiding repeated LLM regeneration and proprietary narrative-feed renewals while retaining a modest price-API bill (e.g., Tiingo or Polygon~\cite{tiingo-pricing,polygon-pricing}) for return labels.
...
\textbf{For traders.} A single forkable repository replaces repeated full-market agent runs. Consensus-gated signals reduce false positives from any one model. Practically, the stack shifts from DIY regeneration (\$73k--\$730k/year of tokens for full-universe coverage) or vendor narrative/alt-data renewals toward contribute ($\sim$\$18/year) or read (\$0 inference) paths, with price history still purchased \`a la carte from EOD/intraday APIs~\cite{tiingo-pricing,polygon-pricing,neudata2024}.
