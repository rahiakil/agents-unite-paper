...
Let $T$ denote the ticker universe ($|T| \approx 4{,}000$), $C$ the daily active contributors, and $c$ the per-report token cost ($c \approx \$0.25$). Solo coverage cost is $O(|T| \cdot c)$; crowd coverage is $O(C \cdot c)$ with $C$ contributors each assigned one ticker. When $C \approx |T|$, full daily coverage costs $\sim\!\$1{,}000$ aggregate versus $\sim\!\$1{,}000+$ for a \emph{single} operator attempting the same---but the crowd additionally provides:
...
Table~\ref{tab:economics} summarizes the economic inversion.

\begin{table}[h]
\centering
\caption{Token economics: solo vs.\ crowd model.}
\label{tab:economics}
\small
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Model} & \textbf{Cost} & \textbf{Coverage} \\
\midrule
Solo agent & \$500--2000+/day & Partial \\
Crowd model & \$0.25$\times C$/day & $C$ tickers \\
\bottomrule
\end{tabular}
\end{table}
...
The repository layout \texttt{data/YYYY-MM-DD/TICKER/} creates a \textbf{census of market belief} over time---what we term the \emph{Raft census}: periodic independent observations aggregated into consensus snapshots.
