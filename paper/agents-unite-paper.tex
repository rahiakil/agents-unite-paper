...
\begin{abstract}
\noindent
Autonomous LLM agents can synthesize market sentiment, news flow, and technical context at scale. Solo deployments hit two walls: token costs that scale linearly with universe coverage, and temporal blind spots no single timezone can close. We present \textbf{Agents Unite}, an open-source architecture that treats a single GitHub repository as a distributed financial memory layer. Each contributor spends about \$0.05 of inference per day on one deterministically assigned ticker; a solo operator covering $\sim$4{,}000 U.S.\ names pays \$200--\$2{,}000 for the same day. Reports land as pull requests. Verifier and consensus agents, coordinated by Raft-style leader election and stake-weighted reputation, merge independent analyses into a canonical ledger. Git workflows stand in for blockchain infrastructure: CI for validation, CODEOWNERS for trust, immutable prompt hashes for audit. The archive supports backtesting, RAG, and longitudinal evaluation across equities, crypto, ETFs, and bonds. Readers who fork the ledger pay nothing. Per-operator token burden drops by four orders of magnitude. History becomes the moat. We propose an \emph{Open Financial Memory Foundation}, Apache-style but scoped to verifiable market intelligence, to steward it.
\end{abstract}
\vspace{0.5em}
\end{@twocolumnfalse}
]
...
This paper formalizes the architecture, draws analogies to distributed consensus and proof-of-stake, and outlines governance via an Apache-style foundation. The biggest moat is not the code. It is history.
...
For traders, one forkable ledger replaces repeated full-market agent runs. Consensus gating cuts single-model false positives. The cost arithmetic lands in Section~\ref{sec:monetary}: readers shift from DIY regeneration or vendor renewals toward contribute-or-fork paths, while price history stays \`a la carte.

For agent builders, canonical prompts and harness adapters create a portable reputation. A contributor who consistently merges accurate calls carries that record across employers and forks.

For society, the stakes are access. Narrative features locked behind terminal subscriptions become versioned and free to read. Global timezone coverage turns contributor geography into infrastructure.

The longer-term opportunity is a narrative ledger: a dated, source-linked record of how market explanations, themes, and disagreements evolved, rather than a collection of isolated scores. Such a ledger can preserve the context that disappears when an agent session ends and make later questions, audits, and backtests answerable against the information available at the time.

Neutral stewardship is also a future proposal, not a present dependency. If the ledger reaches broad multi-asset participation, an \textbf{Open Financial Memory Foundation (OFMF)} could adopt Apache-style governance~\cite{apache}: individual merit over corporate affiliation, protected core workflows, and tagged releases for reproducible research. Whether to form such an entity should follow demonstrated contributor demand and governance design work.
