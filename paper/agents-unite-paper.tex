...
\author{
  Parag Paul\thanks{RIG AI; \texttt{parag@rigai.co}} \and
  Wei-Shinn Ku\thanks{Department of Computer Science and Software Engineering, Auburn University, Auburn, AL, USA.}
}
\date{}
...
Table~\ref{tab:related} summarizes how Agents Unite differs from contemporary decentralized AI memory and inference systems on three axes: Git-native deployability, financial domain focus, and live contribution.
...
The trade-off is Git-native deployability over on-chain finality (Section~\ref{sec:git-substrate}), with optional Phase~4 anchoring for tamper evidence.
...
\begin{itemize}
  ...
  \item \textbf{Focus diversity}: randomized slices (sentiment, news, social, trading) turn assignment collisions into complementary reports (Section~\ref{sec:specialization}).
\end{itemize}

\subsection{Cost Model}

Table~\ref{tab:costmodel} reports \textbf{measured} token counts from the 35 reports in the live repository (Section~\ref{sec:experiments}) and derives per-report cost from published per-million-token rates. Solo scenarios multiply by $|T|$; the crowd scenario multiplies by one assignment per contributor.

\begin{table}[h]
\centering
\caption{Measured per-report cost on 35 live reports (July 2026 API rates, 5$\times$ harness overhead).}
\label{tab:costmodel}
\scriptsize
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Metric} & \textbf{Mean} & \textbf{p90} \\
\midrule
Input tokens (report + sources) & 297 & 635 \\
Output tokens (report body) & 188 & -- \\
\midrule
Cost on GPT-4o-mini~\cite{openai-pricing} & \$0.00034 & \$0.00067 \\
Cost on DeepSeek V4-Flash~\cite{deepseek-pricing} & \$0.00026 & \$0.00052 \\
Cost on Gemini 2.5 Flash-Lite~\cite{gemini-pricing} & \$0.00022 & \$0.00045 \\
Cost on GPT-4.1~\cite{openai-pricing} & \$0.00448 & \$0.00898 \\
\midrule
\textbf{End-to-end budget ceiling} & \multicolumn{2}{c}{\textbf{$\sim$\$0.05}} \\
\bottomrule
\end{tabular}
\end{table}
...
\scriptsize
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{3.5cm}>{\raggedright\arraybackslash}p{2.7cm}>{\raggedright\arraybackslash}p{1.4cm}@{}}
\toprule
...
Retail EOD API (Tiingo) & Vendor pricing~\cite{tiingo-pricing} & 360 \\
Intraday/history API (Polygon) & Vendor pricing~\cite{polygon-pricing} & 2{,}388 \\
Solo LLM (budget) & \$200/day $\times$ 365 (Table~\ref{tab:api-rates}) & $\sim$73{,}000 \\
Solo LLM (mid) & \$600/day $\times$ 365 & $\sim$219{,}000 \\
...
Figure~\ref{fig:arch} depicts the end-to-end pipeline. There is no central application server; \textbf{GitHub is the database}. Three agent roles do the work, assigned per cycle by a hash lottery:

\begin{itemize}
  \item \textbf{Researcher agents} browse sources, synthesize narrative and price context, and emit a structured report with a bounded \texttt{sentiment\_score} $\in [-1,1]$, theme bullets, and cited URLs.
  \item \textbf{Verifier agents} inspect a posted PR rather than re-deriving the report. They run cheap yes/no and cross-encoder checks against cited URLs (Section~\ref{sec:cheap-verify}) and approve, request changes, or reject.
  \item \textbf{Consensus agents} collect the day's validated reports for an assigned ticker, run the weighted-median aggregation in Section~\ref{sec:weighted-median}, and merge the canonical \texttt{consensus.md}.
\end{itemize}
...
\caption{Distributed ingestion pipeline. Each contributor machine runs assignment, research, and validation locally; GitHub CI and peer agents provide trust anchors. Verifier and consensus roles act on public PRs after submission.}
\label{fig:arch}
\end{figure}
...
\subsection{Agent Specialization and Analysis Modalities}
...
Agents Unite is designed to grow in five phases. Each phase adds a capability without invalidating earlier history; Table~\ref{tab:phases} summarizes the roadmap.

\begin{table}[h]
\centering
\caption{Five-phase rollout roadmap.}
\label{tab:phases}
\scriptsize
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.10\columnwidth}>{\raggedright\arraybackslash}p{0.25\columnwidth}>{\raggedright\arraybackslash}p{0.55\columnwidth}@{}}
\toprule
\textbf{Phase} & \textbf{Cadence / scope} & \textbf{What lands} \\
\midrule
1 & Daily, U.S.\ equities & Crowd ingestion, CI guards, verifier and consensus merge, reputation in \texttt{reputation.json} (no token). \\
2 & Hourly + RAG index & Hourly shards, embedded report index and \texttt{wiki/} knowledge graph for downstream queries. \\
3 & Hourly leader election & Deterministic Raft-style leader per \texttt{(date, ticker, hour)} shard, PR lifecycle referee, source-diversity weighting calibration. \\
...
A single agent report is noisy. Agents Unite implements a \textbf{three-stage consensus pipeline} inspired by Raft~\cite{raft} and proof-of-stake validator networks~\cite{ethereum-pos}: research agents produce scored reports, verifier agents validate them on the PR, and consensus agents aggregate the validated scores into \texttt{consensus.md}. Figure~\ref{fig:pipeline} shows the role pipeline.
...
\label{sec:weighted-median}

Before aggregation, each research agent emits a \texttt{sentiment\_score} $\in [-1,1]$ in the report's YAML frontmatter (Section~4.4), grounded in cited sources and modality-specific synthesis. After verifiers approve the PR on the public repo, the day's validated reports for a given ticker carry a set of scores $\{x_1, \ldots, x_n\}$, one per contributing research agent. The consensus agent's job is to turn those validated scores into a single canonical \texttt{consensus\_score} plus a preserved divergence record.
...
Section~\ref{fig:raft} fixes \emph{who} may merge an hourly shard; here we specify \emph{when} and \emph{what} is recorded.

\begin{enumerate}[label=\textbf{\arabic*.}]
  \item \textbf{Submit.} After local validation, the contributor opens a public PR. Verifiers for that cycle inspect the report, cited URLs, and \texttt{prompt\_hash} on the PR itself.
  \item \textbf{Accumulate.} Multiple PRs for the same \texttt{(date, ticker, hour)} may arrive during the window. The leader is chosen at close, not on first arrival.
  \item \textbf{Referee.} At hour end plus a short grace period, CI gathers PRs that passed schema and source checks, recomputes the minimum hash, merges the winner, and closes the rest with a pointer to the canonical report. Non-leader themes may still appear as attributed comments in \texttt{consensus.md}.
  \item \textbf{Record.} Provenance spans three layers: the PR body and sources, the merge commit (SHA, author, CI checks), and per-ticker \texttt{provenance.json} listing contributors, verifier approvals, and the leader hash.
\end{enumerate}

The winning PR may contain the leader's own report or, if they hold the consensus role, the shard's aggregated \texttt{consensus.md}. Leadership is hash-determined, not a standing privilege; GitHub serializes merges after CI names the winner.
...
We introduce the term \textbf{Raft census} to describe the periodic aggregation of contributor-local observations into consensus snapshots over calendar time. Unlike a one-shot sentiment poll, the census has four properties:

\begin{enumerate}[label=\arabic*.]
  \item \textbf{Repeated}: daily (Phase~1), hourly (Phase~2+) observations per ticker;
  \item \textbf{Locally generated}: each report is produced in the contributor's local clone before a public pull request is opened; merged history and open PRs are readable, so the design does not claim Estimize-style blind submission (Section~\ref{sec:estimize});
...
%=============================================================================
\section{Downstream Use of the Ledger}
\label{sec:downstream}
%=============================================================================

The census produces a versioned history under \texttt{data/YYYY-MM-DD/TICKER/}. Researchers and agents consume that archive after merge; backtesting and RAG are not consensus steps.
...
\begin{enumerate}[label=(\alph*)]
  \item Load reports via the public repository's \texttt{examples/load\_reports.py} helper for ticker $t$ and window $[t_0, t_1]$;
...
\subsection{RAG and LLM Queries}
...
We benchmark the live system against the real 291-ticker universe and 35 committed reports. The harness (\texttt{experiments/run\_experiments.py}, output in \texttt{experiments/results.json}) invokes the production assignment algorithm, the real validator, and measured report artifacts, so the numbers below are reproducible from the repository. We address five questions; the forward-looking backtest and federation studies that require a longer observation window remain in our evaluation roadmap.

\subsection{E1. Does the crowd cover the universe faster than a solo agent?}

We simulate the deterministic assignment over $C \in \{10,50,100,200,500,1000,2000,3000\}$ contributors for 30 days. Figure~\ref{fig:coverage} shows coverage saturating at 100\% with $C=100$ (30 days), while a single contributor over a full year reaches only \textbf{70.8\%} (206 of 291 tickers) because the deterministic hash re-assigns the same ticker repeatedly. The crowd crosses full coverage roughly two orders of magnitude faster in contributor-days.

\begin{figure}[h]
\centering
\begin{tikzpicture}[x=0.011cm, y=0.04cm]
  \draw[->] (0,0) -- (3150,0) node[right,font=\scriptsize] {Contributors};
  \draw[->] (0,0) -- (0,108) node[above,font=\scriptsize] {Coverage \%};
  \foreach \x/\l in {0/0,500/500,1000/1K,1500/,2000/2K,2500/,3000/3K}
    \draw (\x,0)--(\x,-2) node[below,font=\tiny] {\l};
  \foreach \y/\l in {0/0,25/25,50/50,75/75,100/100}
    \draw (0,\y)--(-60,\y) node[left,font=\tiny] {\l};
  \draw[blue!60, thick] plot coordinates
    {(10,62.2)(50,99.66)(100,100)(200,100)(500,100)(1000,100)(2000,100)(3000,100)};
  \fill[blue!60] (10,62.2) circle (2pt) (50,99.66) circle (2pt)
    (100,100) circle (2pt) (200,100) circle (2pt) (500,100) circle (2pt)
    (1000,100) circle (2pt) (2000,100) circle (2pt) (3000,100) circle (2pt);
  \draw[red!70, dashed, thick] (0,70.8)--(3150,70.8)
    node[right,font=\tiny,black] {solo, 365d: 70.8\%};
\end{tikzpicture}
\caption{Universe coverage vs.\ contributors over 30 days (real assignment, 291 tickers). The crowd reaches 100\% at $C{=}100$; a solo agent plateaus at 70.8\% even after a full year.}
\label{fig:coverage}
\end{figure}

\subsection{E2. Does the coverage optimizer reduce redundant work?}

The optimizer weights zero-coverage tickers 10$\times$. Coverage parity is preserved, but \emph{collisions} (multiple contributors on the same ticker) drop sharply. At $C=100$ the optimizer produces 23 collisions versus 468 for uniform hashing, a \textbf{20$\times$ reduction}; at $C=50$ it is 25$\times$ (4 vs.\ 103). Figure~\ref{fig:collisions} plots collision rate. Fewer collisions mean fewer duplicate PRs and more complementary focus slices.

\begin{figure}[h]
\centering
\begin{tikzpicture}[x=0.011cm, y=120cm]
  \draw[->] (0,0) -- (3150,0) node[right,font=\scriptsize] {Contributors};
  \draw[->] (0,0) -- (0,0.95) node[above,font=\scriptsize] {Collision rate};
  \foreach \x/\l in {0/0,1000/1K,2000/2K,3000/3K}
    \draw (\x,0)--(\x,-0.02) node[below,font=\tiny] {\l};
  \foreach \y/\l in {0/0,0.25/,0.5/.5,0.75/,0.9/.9}
    \draw (0,\y)--(-60,\y) node[left,font=\tiny] {\l};
  \draw[blue!60, thick] plot coordinates
    {(10,0.0)(50,0.0027)(100,0.0077)(200,0.0265)(500,0.418)(1000,0.709)(2000,0.8545)(3000,0.903)};
  \draw[red!70, thick, dashed] plot coordinates
    {(10,0.0167)(50,0.0687)(100,0.156)(200,0.2733)(500,0.5221)(1000,0.7183)(2000,0.8547)(3000,0.903)};
  \node[font=\tiny, blue!60!black] at (2600,0.20) {optimizer};
  \node[font=\tiny, red!70!black] at (2600,0.62) {uniform};
\end{tikzpicture}
\caption{Collision rate vs.\ contributors. The coverage optimizer (solid) keeps collisions low while coverage matches uniform hashing (dashed).}
\label{fig:collisions}
\end{figure}

\subsection{E3. Is the hash assignment statistically uniform?}

Assigning 10{,}000 synthetic contributors on a single day yields a per-ticker count of mean 34.36 (expected 34.36) with $\sigma{=}5.97$. A $\chi^2$ goodness-of-fit statistic of \textbf{300.6} falls below the critical value 340.0 at $\alpha{=}0.01$ (290 d.f.), so we fail to reject uniformity. Birthday-problem collision probabilities rise as expected: 14.5\% at $C{=}10$, 98.9\% at $C{=}50$, and 100\% at $C{\geq}100$, motivating the consensus layer for collision reconciliation rather than conflict avoidance.

\subsection{E4. Does MAD outlier rejection improve consensus under attack?}

Using the 34 real sentiment scores (mean 0.116, $\sigma$ 0.269) as a base distribution, we inject a coordinated +0.6 ``pump'' into 20\% of reports and compute the weighted median with and without the MAD filter over 500 trials per $n$. Table~\ref{tab:consensus} and Figure~\ref{fig:consensus} show mean absolute error (MAE) falling with $n$, and the MAD filter improving accuracy by \textbf{14.2\%} at $n{=}10$, 23.1\% at $n{=}20$, and \textbf{47.0\%} at $n{=}50$. At $n{=}3$ the filter slightly hurts ($-8.9\%$), confirming the design rule to apply it cautiously for small $n$.

\begin{table}[h]
\centering
\caption{Consensus MAE vs.\ report count $n$ under a 20\% coordinated pump.}
\label{tab:consensus}
\scriptsize
\begin{tabular}{@{}rrrr@{}}
\toprule
\textbf{$n$} & \textbf{MAE no filter} & \textbf{MAE + MAD} & \textbf{Improv.} \\
\midrule
1  & 0.1124 & 0.1124 & 0.0\% \\
2  & 0.0799 & 0.0799 & 0.0\% \\
3  & 0.0730 & 0.0795 & $-8.9\%$ \\
...
\begin{figure}[h]
\centering
\begin{tikzpicture}[x=1.1cm, y=18cm]
  \draw[->] (0,0) -- (6.2,0) node[right,font=\scriptsize] {reports $n$};
  \draw[->] (0,0) -- (0,0.13) node[above,font=\scriptsize] {MAE};
  \foreach \x/\l in {1/1,2/2,3/3,5/5,10/10,20/20,50/50}
    \draw (\x,0)--(\x,-0.005) node[below,font=\tiny] {\l};
  \foreach \y/\l in {0/0,0.03/.03,0.06/.06,0.09/.09,0.12/.12}
    \draw (0,\y)--(-0.12,\y) node[left,font=\tiny] {\l};
  \draw[red!70, thick] plot coordinates
    {(1,0.1124)(2,0.0799)(3,0.073)(5,0.0737)(10,0.0573)(20,0.0472)(50,0.0452)};
  \draw[blue!60, thick] plot coordinates
    {(1,0.1124)(2,0.0799)(3,0.0795)(5,0.0732)(10,0.0491)(20,0.0363)(50,0.0239)};
  \node[font=\tiny, red!70!black] at (4.0,0.060) {no filter};
  \node[font=\tiny, blue!60!black] at (4.0,0.030) {+ MAD};
\end{tikzpicture}
\caption{Consensus error vs.\ report count. MAD outlier rejection (solid blue) increasingly beats the raw median as $n$ grows under a coordinated pump.}
\label{fig:consensus}
\end{figure}

\subsection{E5. Does the CI validation gate scale?}

Timing the production \texttt{validate\_report.py} over all 37 report directories yields \textbf{772 reports/sec} (1.3 ms/report). Projected to the 3{,}000 PRs/day target, full schema validation completes in $\sim$3.9 seconds, negligible relative to GitHub Actions scheduling latency. Validation is fully offline, so throughput is CPU-bound and scales linearly with CI runners.

\subsection{Measured report cost}

The 35 committed reports average 297 input and 188 output tokens. At July 2026 API rates with a 5$\times$ harness overhead, the report artifact costs \$0.00034 on GPT-4o-mini, \$0.00026 on DeepSeek V4-Flash, and \$0.00022 on Gemini 2.5 Flash-Lite; the p90 is \$0.00067. We retain a conservative \$0.05 end-to-end ceiling to absorb web search and multi-step reasoning, but the persisted artifact itself is effectively free.
...
The experiments in Section~\ref{sec:experiments} validate assignment, consensus, cost, and validation. The following studies require a longer observation window or a public benchmark and remain future work:
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item \textbf{Shared-ledger reuse.} At a fixed false-accept rate, compare recall@1 for relevant historical answers retrieved from the shared ledger with a single participant's cache.
  ...
  \item \textbf{Source-diversity weighting.} Compare equal-weight, reputation-only, and source-diversity-weighted aggregation on held-out dates, ablating URL-domain, modality, and model-family diversity independently.
  \item \textbf{Adversarial probes.} Measure prompt-injection acceptance and coordinated Sybil influence under the CI and verifier gates.
\end{enumerate}
